...
const Courses = (props) => {
  ...
  const handleEditClick = async (course) => {
    ...
    setFormData({
      ...
      title: course.title,
      prereqs:
        Array.isArray(course.prereq_groups) && course.prereq_groups.length > 0
          ? course.prereq_groups.map((group) =>
              Array.isArray(group) ? group : []
            )
          : [[]],
      seasons: course.seasons,
      coreqs:
        Array.isArray(course.coreq_groups) && course.coreq_groups.length > 0
          ? course.coreq_groups.map((group) =>
              Array.isArray(group) ? group : []
            )
          : [[]],
  ...
  const class_options = classes.map((cls) => {
    ...
    return {
      value: cls.id,
      label: `${major?.abbreviation || "N/A"} ${cls.class_number} - ${
        cls.title
      }`, // Updated label
    };
  });
      ...
      <Dialog fullWidth open={openDialog} onClose={handleClose}>
        ...
        <DialogContent>
          ...
          <form onSubmit={handleSubmit}>
            ...
            {(Array.isArray(formData.prereqs) ? formData.prereqs : []).map(
              (group, groupIndex) => (
                <div key={groupIndex} style={{ marginBottom: "10px" }}>
                  <Autocomplete
                    ...
                    onChange={(e, newValue) => {
                      ...
                      updatedGroups[groupIndex] = newValue.map(
                        (option) => option.value
                      );
                      setFormData((prev) => ({
                        ...prev,
                    ...
                    renderInput={(params) => (
                      <TextField
                        {...params}
                        label={`Prerequisite Group ${groupIndex + 1}`}
                        variant="outlined"
                        fullWidth
                      />
                    )}
            ...
            {(Array.isArray(formData.coreqs) ? formData.coreqs : []).map(
              (group, groupIndex) => (
                <div key={groupIndex} style={{ marginBottom: "10px" }}>
                  <Autocomplete
                    multiple
                    ...
                    renderInput={(params) => (
                      <TextField
                        {...params}
                        label={`Corequisite Group ${groupIndex + 1}`}
                        variant="outlined"
                        fullWidth
                      />
                    )}
                        ...
                        >
                          ...
                          >
                            <Box sx={{ margin: 1 }}>
                              ...
                              <div>
                                <b>Prerequisites:</b>{" "}
                                {Array.isArray(row.prereq_groups) &&
                                row.prereq_groups.some(
                              ...
                              <div>
                                <b>Corequisites:</b>{" "}
                                {Array.isArray(row.coreq_groups) &&
                                row.coreq_groups.some(
                                  (group) => group.length > 0
                                )
                                  ? row.coreq_groups
                                      .filter((group) => group.length > 0)
